...
const TEST_TAGS = [['#CAP', '#분산'], ['#TCP', '#Security'], ['#Indexing', '#Storage'], ['#OS', '#Memory'], ['#Graph', '#DP']];

function getTestTags(id: number) { return TEST_TAGS[id % TEST_TAGS.length]; }
...
function Table({ items, totalElements, page, totalPages, onPageChange, onRefresh }: Props) {
    const start = items.length === 0 ? 0 : (page - 1) * items.length + 1;
    const end = start + items.length - 1;

    const handleClickDelete = (id: number) => {
        if (!confirm('삭제하시겠습니까?')) return;
        QuestionService.delete(id).then(() => onRefresh());
    }
        ...
        <div className="bg-surface-container-lowest rounded-3xl overflow-hidden shadow-sm shadow-primary/5">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-surface-container-low">
                <th className="px-8 py-5 font-headline text-xs uppercase tracking-widest text-on-surface-variant font-bold text-center border-x border-outline-variant/60">제목</th>
                <th className="px-6 py-5 font-headline text-xs uppercase tracking-widest text-on-surface-variant font-bold text-center border-x border-outline-variant/60">카테고리</th>
                <th className="px-6 py-5 font-headline text-xs uppercase tracking-widest text-on-surface-variant font-bold text-center border-x border-outline-variant/60">태그</th>
                <th className="px-4 py-5 font-headline text-xs uppercase tracking-widest text-on-surface-variant font-bold text-center w-24 border-x border-outline-variant/60">생성일</th>
                <th className="px-8 py-5 font-headline text-xs uppercase tracking-widest text-on-surface-variant font-bold text-center w-36 border-x border-outline-variant/60">상태</th>
                <th className="px-6 py-5 font-headline text-xs uppercase tracking-widest text-on-surface-variant font-bold text-center w-56 border-x border-outline-variant/60"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-surface-container-low">
              {items.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-8 py-16 text-center text-sm text-on-surface-variant">
                    등록된 문제가 없습니다.
                  </td>
                </tr>
              ) : items.map(item => (
                <tr key={item.id} className="hover:bg-surface-container-low/30 transition-colors group">
                  ...
                  <td className="px-6 py-6 border-x border-outline-variant/60">
                  ...
                  <td className="px-6 py-6 border-x border-outline-variant/60">
                    <div className="flex flex-wrap gap-1.5">
                      {getTestTags(item.id).map(tag => (
                        <span key={tag} className="text-[11px] font-bold text-secondary px-2 py-0.5 rounded-md border border-secondary/20 bg-secondary/5">{tag}</span>
                      ))}
                    </div>
                  </td>
                  ...
                  <td className="py-6 w-56 border-x border-outline-variant/60">
          ...
          <div className="px-8 py-6 flex items-center justify-between border-t border-surface-container-low bg-surface-container-lowest">
            <div className="text-sm text-on-surface-variant font-medium">
              총 <span className="font-bold text-on-surface">{totalElements}</span>개의 문제 중&nbsp;
